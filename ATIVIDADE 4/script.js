...
function renderizarProjetos() {
    // Busca o H1, que agora é o título principal da seção
    ...
    if (projetosSection) {
        ...
        // Adiciona o grid de projetos APÓS o H1 existente
...
async function loadContent(pageUrl) {
    ...
    try {
        ...
        // Move o foco para o novo título principal (h1 ou h2) da página carregada.
        const newPageTitle = DOM.main.querySelector('h1, h2');
        if (newPageTitle) {
            newPageTitle.setAttribute('tabindex', '-1'); // Permite focar o elemento via JS
            newPageTitle.focus(); // Move o foco do leitor de tela
        }
        // ===============================================
 ...
 * (Atualizado com ARIA para WCAG)
 * @param {HTMLElement} inputElement - O campo de formulário.
 * @param {string} message - A mensagem de erro.
 */
function displayError(inputElement, message) {
    let errorDiv = inputElement.nextElementSibling;
    const errorId = inputElement.id + '-error'; // ID único para a mensagem de erro

    if (!errorDiv || !errorDiv.classList.contains('error-message')) {
        errorDiv = document.createElement('div');
        errorDiv.classList.add('error-message');
        errorDiv.id = errorId; // Define o ID da div de erro
    ...
    // --- ARIA (WCAG) ---
    // Informa ao leitor de tela que o campo é inválido
    inputElement.setAttribute('aria-invalid', 'true');
    // Conecta o campo à mensagem de erro
...
function clearError(inputElement) {
    ...
    // --- ARIA (WCAG) ---
    // Informa ao leitor de tela que o campo agora é válido (ou não mais inválido)
    inputElement.setAttribute('aria-invalid', 'false');
    // Remove a conexão com a mensagem de erro
...
function handleFormSubmit(event) {
    ...
    if (isValid) {
        ...
        // Limpa todos os estados de erro/ARIA dos inputs
        clearError(nomeInput);
        clearError(telefoneInput);
        clearError(idadeInput);
        clearError(disponibilidadeSelect);
